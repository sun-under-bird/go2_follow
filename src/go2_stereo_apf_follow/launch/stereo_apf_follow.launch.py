...
def generate_launch_description():
    """启动 UWB 目标适配、双目感知和直接输出 /cmd_vel 的 APF 控制器."""
    pkg_share = get_package_share_directory("go2_stereo_apf_follow")
    default_config = os.path.join(pkg_share, "config", "stereo_apf_follow.yaml")
    ...
    return LaunchDescription(
        [
            ...
            Node(
                package="go2_stereo_apf_follow",
                executable="stereo_apf_controller_node",
                name="stereo_apf_controller_node",
                output="screen",
                parameters=[
                    config_file,
                    {
