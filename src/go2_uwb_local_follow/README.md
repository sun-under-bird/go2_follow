...
视差时间戳通过 TF 转换到 `base_footprint`：`0.08 <= z <= 0.65 m` 的点参与障碍
...
距离误差增大，目标方位在约 `80.2°` 以内时保持边走边转，超过门槛才停止前进
并原地转向。有效非零线速度范围默认设为
`0.25~0.80 m/s`，避免 MCF 长时间接收无法形成步态的极小前进速度。线加速度和
...
完整链路新增安全绕障反馈：只有规划器持续批准前进绕障时，跟随器才允许在约 `80.2°～85.4°`
的目标偏角范围继续绕行。普通跟随超过约 `80.2°` 停止前进，进入对准后回落到约 `71.6°`
才恢复。反馈 `0.20 s` 失效、原地旋转、受阻或急停恢复都会取消许可。
