...
MCF 模式实测原地转向需要超过约 `0.8 rad/s`，因此侧后方盲转上限默认设为
`0.90 rad/s`，总角速度仍限制为 `1.20 rad/s`，并通过 `1.50 rad/s²` 的角加速度
限制平滑升速。提高该参数会缩短转向时间，但也会增大 UWB 方位抖动引起的摆动；
接入避障后，速度仲裁和局部避障应沿用同一角速度上限。

有效非零线速度范围默认设为 `0.12~0.60 m/s`，避免 MCF 长时间接收无法形成
步态的极小前进速度。线加速度和减速度均为 `0.80 m/s²`；从静止加速到最高
线速度约需 0.75 秒。目标丢失属于安全事件，仍绕过普通减速过程并立即发布
零速度。
...
规划器均匀采样线速度和角速度，同时显式加入名义速度、上一指令、停车以及左右
最小有效原地转向。碰撞候选直接淘汰，其余候选按跟随、平滑、障碍净空和前进
代价评分。`/odom_leg`、名义速度或障碍点云任一超时都会故障停车。
...
- `EMERGENCY_STOP`：正前方紧急区有障碍，立即撤销线速度。
