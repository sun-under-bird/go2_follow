...
class SafetyMux(Node):
    ...
    def _tick(self):
        ...
        cloud_timeout = float(self.get_parameter("pointcloud_timeout_sec").value)
        emergency_x_max = float(self.get_parameter("emergency_x_max").value)
        slow_x_max = float(self.get_parameter("slow_x_max").value)
        ...
        if not self.target_valid:
            status = "stop: ONE1000 target invalid or stale"
        elif not self.path_valid:
            status = "stop: no valid local path"
        elif not self._age_ok(self.latest_cloud_time, cloud_timeout):
            status = "stop: stereo point cloud stale"
        elif not self._age_ok(self.latest_cmd_time, cmd_timeout) or self.latest_cmd is None:
            status = "stop: cmd_vel stale"
        elif self.nearest_front_obstacle is not None and self.nearest_front_obstacle <= emergency_x_max:
